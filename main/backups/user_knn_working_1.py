...
parser.add_argument('--weight_on_knn', type=int, default=1)
...
def user_knn(interactions, target_users_file=None, k=50, user_key="user_id", item_key="item_id", rating_key="rating",
    ...
    first_block = pool.apply_async(row_dealer, (pearson_urm, target_users[:step],
                                                uid_position_dic, "One", 0, k,))
    second_block = pool.apply_async(row_dealer, (pearson_urm, target_users[step:2*step],
                                                 uid_position_dic, "Two", step, k,))
    third_block = pool.apply_async(row_dealer, (pearson_urm, target_users[2*step:3*step],
                                                uid_position_dic, "Three", 2*step, k,))
    fourth_block = pool.apply_async(row_dealer, (pearson_urm, target_users[3*step:],
...
def urm_computer(interactions, target_users_file=None, user_key="user_id", item_key="item_id", rating_key="rating",
    ...
    max_usr_average = max(user_average.values())
    ...
    max_item_average = max(item_average.values())
...
def recommend(similarity_matrix, user_rating_matrix, target_users, new_uid_position,
    ...
    for user in target_users:
        ...
        for item_column in interesting_items:
            rankers = set(item_rating_user[item_column])
            # Checks the estimated rating only for valid item_ids
            if position_iid_dic[item_column] not in non_recommendable_set:
                if args.weight_on_knn == 1:
                    weights = 0
                    for ranker in neighbours_columns_set:  # Maybe you can invert the two checks to make it faster
                        if ranker in rankers:
                            weights += similarity_matrix_row[ranker]
                    if weights == 0:
                        print("Eccheccazz")
                        weights = 1
                if args.weight_on_knn == 0:
                    weights = 1
                else:
                    weights = sum(similarity_matrix_row)
...
def hold_out_split(hold_out_percentage, interactions_file, user_key="user_id", item_key="item_id", rating_key="rating",
    ...
    test_target_users = test_target_users[:10000]
...
def main(interactions, target_users_file=None, k=50, user_key="user_id", item_key="item_id", rating_key="rating",
    ...
    # If it's testing then it computes the map and prints it
        ...
        real_map = estimate_quality(rec_dictionary, target_users, rec_length, test_set)
