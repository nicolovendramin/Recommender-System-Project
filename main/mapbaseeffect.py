...
def mapk(actual, predicted, k=10):
    mappo = np.mean([apk(a, p, k) for a, p in zip(actual, predicted)])
    assert 0 <= mappo <= 1
    return mappo
...
def estimate_quality(reco_dictionary, targeto_users, rec_length, test_set):
    ...
    for usero in targeto_users:
        recs = []
        for i in reco_dictionary[usero]:
            recs.append(i[0])
        dirty_map += map_computer(recs, pos_items=test_set[usero], at=rec_length)
        number += 1
...
def main(args):
    ...
    if args.holdout_perc != 0:
        for abracadabra in range(1, 20):
            calculated_map = estimate_quality(rec_dictionary, target_users, abracadabra, removed_ratings_per_user)
            print("\nEstimated map@", abracadabra, ": ",
                  calculated_map)
            o_file = open(args.prediction_file.replace(".csv", "[INFO].txt"), "a")
            o_file.write("\nEstimated map@"+ str(abracadabra) + ": " + str(calculated_map))
