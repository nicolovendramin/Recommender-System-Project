...
def recommend():
    ...
    expired_items = set(check_expiration())
    ...
    time_span = time.time()
    U, V = fit(csr_user_rating_matrix)
...
def fit(R, num_factors=440, reg=0.015, iters=6, scaling='linear', alpha=40, epsilon=1e-6, init_mean=0.0, init_std=0.1,
        rnd_seed=42):
    ...
    if scaling == 'linear':
        C = _linear_scaling(R, alpha)
    else:
        C = _log_scaling(R, alpha, epsilon)
    ...
    for it in range(iters):
        ...
        print(it + 1, " iterations done ")
    return X, Y


def _linear_scaling(R, alpha):
    C = R.copy().tocsr()
    C.data *= alpha
    C.data += 1.0
    return C
...
def _log_scaling(R, alpha, epsilon):
    C = R.copy().tocsr()
