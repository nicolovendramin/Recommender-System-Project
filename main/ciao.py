from mf import BPRMF_sgd, FunkSVD_sgd
...
def recommend():
    ...
    time_span = time.time()
    U, V = BPRMF_sgd(csr_user_rating_matrix, num_factors=50, lrate=0.01, user_reg=0.015,
                     pos_reg=0.015, neg_reg=0.0015, iters=10, sample_with_replacement=True,
                     use_resampling=False,  init_mean=0.0, init_std=0.1, lrate_decay=1.0, rnd_seed=42)
    ...
    for target_user in target_users:
        ...
        if counter % 500 == 0:
            print(str(int(counter*100/total_number))+"% ("+str(int(non_rec_counter*100/counter))+"%)"),
    ...
    if args.holdout_perc != 0:
        ...
        map_at = \
            estimate_quality(rec_dictionary, target_users, rec_length=args.rec_length, test_set=removed_ratings_per_user)
