...
def apply_shrinkage(dist, urm, user=0):
    x_ind = urm.copy()
...
print("Technique : Item User Pipeline ")
o_file.write("Technique : Item User Pipeline ")
...
item_item_similarity_matrix = rating_user_matrix.dot(user_rating_matrix)
...
for target_user in target_users:
    counter += 1
    print("Enlarging the model for user number: "+str(counter))
    if target_user in user_rated_items.keys():
        ...
        non_recommendable_set = set(expired_items)
        abso = set(user_rated_items[target_user])
        ...
        for i in k_best_matches:
            if i in non_zeros:
                if position_iid_dic[i] in non_recommendable_set and position_iid_dic[i] not in abso:
...
for key in rec_dictionary.keys():
    for i in rec_dictionary[key]:
        rows.append(uid_position_dic[key])
        columns.append(iid_position_dic[i[0]])
        data.append(1)
...
if args.shrink_factor > 0:
    user_user_similarity_matrix = apply_shrinkage(user_user_similarity_matrix, user_rating_matrix,1)
