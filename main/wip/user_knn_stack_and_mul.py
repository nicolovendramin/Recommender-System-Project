...
parser.add_argument('--weights_policy', type=str, default="constant")
...
def user_knn(interactions, target_users_file=None, k=50, user_key="user_id", item_key="item_id", rating_key="rating",
    ...
    user_rating_matrix, target_users, users, position_uid_dic, uid_position_dic, position_iid_dic, iid_position_dic,\
        row_number, col_number, user_rated_items, item_rating_user, pearson_urm, removed_positions\
    ...
    for i in range(0, args.number_of_cpu):
        ...
        if i == args.number_of_cpu:
            end = tot
        blocks.append(pool.apply_async(row_dealer, (pearson_urm, target_users[start:end],
                                                    uid_position_dic, str(i), step*i, k,)))
    ...
    similar_users_id = dict(results[0][4])
    new_uid_position = dict(results[0][3])
    ...
    for i in range(1, args.number_of_cpu):
        ...
        similar_users_id.update(results[i][4])
        new_uid_position.update(results[i][3])
...
def row_dealer(user_rating_matrix, target_users, uid_position_dic, task_name, offset=0, k=50):
    ...
    for user in target_users:
        ...
        similarity_product_array = np.asarray(row.dot(transposed_urm).toarray()[0])
...
def data_importation(interactions, target_users_file=None, user_key="user_id", item_key="item_id", rating_key="rating"):
    ...
    for i, row in interactions_reader.iterrows():
        ...
        elif args.rating_policy == "boolean" or True:
            temp_dic[key] = 1
...
def recommend(similarity_matrix, user_rating_matrix, target_users, new_uid_position,
    ...
    for user in target_users:
        ...
        neighbours_columns_set = set(sim_sparse_row.nonzero()[1])
        for column in neighbours_columns_set:
            column_user = position_uid_dic[column]
            if column_user in uri_keys:
                rated_items = user_rated_items[column_user]
                interesting_items.extend(rated_items)
        ...
        used_items = []
        item_matrix_reduced = None
        for i in range(0, len(interesting_items)):
            item_column = interesting_items[i]
            # Checks the estimated rating only for valid item_ids
            if position_iid_dic[item_column] not in non_recommendable_set:
                used_items.append(position_iid_dic[item_column])
                if item_matrix_reduced is None:
                    item_matrix_reduced = rating_user_matrix.getrow(item_column).T
                else:
                    item_matrix_reduced = sps.hstack((item_matrix_reduced, rating_user_matrix.getrow(item_column).transpose(copy=True)))

        estimated_ratings = []
        if item_matrix_reduced is not None:
            estimated_ratings = np.asarray((sim_sparse_row.dot(item_matrix_reduced).todense())).ravel()
        if len(estimated_ratings) == len(used_items):
            for i in range(0, len(used_items)):
                rec_dictionary[user].append((used_items[i], estimated_ratings[i]))
...
def main(interactions, target_users_file=None, k=50, user_key="user_id", item_key="item_id", rating_key="rating",
    ...
    target_users, similarity_matrix, new_uid_position, user_rating_matrix, position_iid_dic, \
        ...
        user_rated_items, item_rating_user, test_set = user_knn(interactions, target_users_file, k, user_key, item_key,
    ...
    # If it's testing then it computes the map and prints it
        ...
        output_file = open(args.prediction_file.replace(".csv", "[INFO].csv"), "a")
        output_file.write("The map estimated was-->MAP@" + str(rec_length) + ": " + str(real_map))
        output_file.close()
...
# Just a customized help callable with the help_me argument
if args.help_me != 19835:
    print('   --rating_file : to indicate the path of the file with interactions  ' +
          ...
          '\n   --rating_policy : is in {sum, max, boolean} sum sums interactions, max picks the maximum '
          'and boolean consider 1 if there is at least one otherwise 0')
else:
    o_file = open(args.prediction_file.replace(".csv", "[INFO].csv"), "w+")
    if args.holdout_perc != 0:
