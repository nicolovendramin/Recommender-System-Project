...
def print_run_details():
    ...
    print("Technique : Item Based SLIM")
    o_file.write("Technique : Item Based SLIM\n")
...
def main():
    ...
    item_item_similarity_matrix, user_rated_items, target_users, user_rating_matrix_non_normalized, \
...
def similarity_computer():
    ...
    assert len(items) == user_rating_matrix.shape[1]
    item_item_similarity_matrix = slim_fit(user_rating_matrix, len(items))

    if args.shrink_factor > 0:
        item_item_similarity_matrix = apply_shrinkage(item_item_similarity_matrix, user_rating_matrix)
...
def slim_fit(user_rating_matrix, number_of_items):
    l1_penalty = 0.01
    l2_penalty = 0.01
    l1_ratio = l1_penalty / (l1_penalty + l2_penalty)
    print(user_rating_matrix.shape)
    positive_only = True
    urm = check_matrix(user_rating_matrix.copy(), 'csc', dtype=np.float32)
    rum = check_matrix(user_rating_matrix.transpose(copy=True), 'csc', dtype=np.float32)

    # initialize the ElasticNet model
    model = ElasticNet(alpha=1.0, l1_ratio=l1_ratio, positive=positive_only, fit_intercept=False, copy_X=False)
    # we'll store the W matrix into a sparse csr_matrix
    # let's initialize the vectors used by the sparse.csc_matrix constructor
    values, rows, cols = [], [], []
    las_time = time.time()
    counter = 0
    tot = int(number_of_items/500)
    avg = 0
    # fit each item's factors sequentially (not in parallel)
    for j in range(number_of_items):
        if j % 1000 == 0:
            now = time.time()
            diff = int(now-las_time)
            las_time = now
            if counter != 0:
                avg = ((counter-1) * avg + diff) / counter
            remaining = (tot-counter) * avg
            counter += 1
            print(str(j)+"] estimated time needed to end: "
                  + str(int(remaining / 60 / 60))+" hours "+str(int(remaining / 60 % 60))+
                  " minutes and "+str(int(remaining) % 60)+" seconds")
        # get the target column
        y = rum.getrow(j).todense().tolist()[0]
        # set the j-th column of the urm to zero
        start_ptr = urm.indptr[j]
        end_ptr = urm.indptr[j+1]
        bak = urm.data[start_ptr: end_ptr].copy()
        urm.data[start_ptr: end_ptr] = 0.0
        ...
        rows.extend(np.arange(number_of_items)[nnz_idx])
        cols.extend(np.ones(nnz_idx.sum()) * j)

        # finally, replace the original values of the j-th column
        urm.data[start_ptr:end_ptr] = bak

    # generate the sparse weight matrix
    return sps.csc_matrix((values, (rows, cols)), shape=(number_of_items, number_of_items), dtype=np.float32)
