...
# do ucb similarity
# enlarge temp dic with suggestions for cold start users
# do recommendations for all the users using icb
# reccomend everyone with ubcf
...
def print_run_details():
    ...
    print("Technique : User Based Collaborative Filtering after User Content Based for cold starters and "
          "users with 1 interaction followed by an Item Content Based enlargement of the URM")
    o_file.write("Technique : User Based Collaborative Filtering after User Content Based for cold starters and "
                 "users with 1 interaction followed by an Item Content Based enlargement of the URM\n")
...
def main():
    ...
    # get the real temporary dictionary
    temp_dic, user_rated_items, target_users, uid_position_dic, user_counter, position_iid_dic, \
        removed_ratings_per_user, users, item_counter, user_counter, iid_position_dic, items = similarity_computer()
    # build the first version of the user rating matrix
    user_rating_matrix, user_rating_matrix_non_normalized = get_final_similarity(temp_dic, user_counter, item_counter)
    # obtain the content based user rating matrix
    ucb_similarity_matrix = user_content_based_similarity_computer(users, target_users, uid_position_dic, item_counter,
                                                                   user_counter)
    # enlarge the temp dic suggesting content based recommendations to the cold starters
    temp_dic = cold_start(target_users, uid_position_dic, ucb_similarity_matrix, user_rating_matrix_non_normalized,
                          expired_items, user_counter, user_rated_items, position_iid_dic, temp_dic, iid_position_dic)
    # recompute the modified temporary dictionary and user rating matrix
    user_user_similarity_matrix, user_rating_matrix_non_normalized = get_final_similarity(temp_dic, user_counter, item_counter)
    """""
    # compute the item content based smilarity matrix
    icb_similarity_matrix = icb_similarity_computer(iid_position_dic, items, item_counter)
    # add content based recommendations to the temporary dictionary
    temp_dic = enlarge_base(target_users, uid_position_dic, icb_similarity_matrix,
                            user_rating_matrix_non_normalized, expired_items, user_rated_items,
                            position_iid_dic, temp_dic, iid_position_dic)
    # compute the final similarity matrix to be used for the very recommendations
    user_user_similarity_matrix, user_rating_matrix_non_normalized = \
        get_final_similarity(temp_dic, user_counter, item_counter)
    """
    # Finally does recommendations
...
def icb_similarity_computer(iid_position_dic, items, item_counter):
    ...
    data_sim, row_indices, col_indices = \
        icb_row_dealer(items, item_content_matrix, content_item_matrix, iid_position_dic)
...
def icb_row_dealer(items, item_content_matrix, content_item_matrix, iid_position_dic, k=args.k):
...
def user_content_based_similarity_computer(users, target_users, uid_position_dic, item_counter, user_counter):
    ...
    data_sim, row_indices, col_indices = \
        ucb_row_dealer(target_users, user_content_matrix, content_user_matrix, uid_position_dic)
...
def similarity_computer():
    ...
    return temp_dic, user_rated_items, target_users, uid_position_dic, user_counter, position_iid_dic, \
        removed_ratings_per_user, users, item_counter, user_counter, iid_position_dic, items


def get_final_similarity(temp_dic, user_counter, item_counter):
    ...
    return user_user_similarity_matrix, user_rating_matrix_non_normalized
...
def recommend(target_users, uid_position_dic, user_user_similarity_matrix, user_rating_matrix_non_normalized,
    ...
    for target_user in target_users:
        counter += 1
        print(str(counter)),
        if target_user not in user_rated_items.keys():
            user_rated_items[target_users] = []
        position = uid_position_dic[target_user]
        filters = []
        for filler in range(0, args.k):
            filters.append(1)
...
def ucb_row_dealer(target_users, user_content_matrix, content_user_matrix, uid_position_dic, k=args.k):
    # Instantiate all the variables needed to collect the results
    data = []
    row_ind = []
    col_ind = []
...
def enlarge_base(target_users, uid_position_dic, item_item_similarity_matrix, user_rating_matrix_non_normalized,
                 expired_items, user_rated_items, position_iid_dic, temp_dic, iid_position_dic):
    counter = 0
    for target_user in target_users:
        if target_user not in user_rated_items.keys():
            rated = []
        else:
            rated = user_rated_items[target_user]
        counter += 1
        print("ICB, ", counter)
        position = uid_position_dic[target_user]
        ...
        k_best_matches = np.argpartition(estimated_ratings, -args.to_keep)[-args.to_keep:]
        non_zeros = list(set(np.nonzero(estimated_ratings)[0]))
        non_recommendable_set = set(it.chain(expired_items, rated))
        # The function returns the 5 top pops more suitable for the user (not expired and not already seen)
        possible_matches = non_personalized_recommendation(args.rec_length, non_recommendable_set)
        for i in k_best_matches:
            if i in non_zeros:
        ...
        # Sorting the matches
        possible_matches.sort(key=lambda tup: tup[1], reverse=True)
        # adding the best as rated items 10 can be adjusted
        for i in possible_matches[:5]:
            temp_dic[position, iid_position_dic[i[0]]] = 1

    return temp_dic


def cold_start(target_users, uid_position_dic, user_user_similarity_matrix, user_rating_matrix_non_normalized,
               expired_items, user_counter, user_rated_items, position_iid_dic, temp_dic, iid_position_dic):
    counter = 0
    flagged = 0
    flag = False
    for target_user in target_users:
        ...
        if flag:
            ...
            # Sorting the matches
            possible_matches.sort(key=lambda tup: tup[1], reverse=True)
            # adding to the temp dic the fake interactions
            for i in possible_matches[:10]:
                temp_dic[position, iid_position_dic[i[0]]] = 1
